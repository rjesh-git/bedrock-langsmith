...
```bash
bedrock-langsmith-trace$ sam build --use-container
```
...
```bash
bedrock-langsmith-trace$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
bedrock-langsmith-trace$ sam local start-api
bedrock-langsmith-trace$ curl http://localhost:3000/
```
...
```bash
bedrock-langsmith-trace$ sam logs -n HelloWorldFunction --stack-name bedrock-langsmith-trace --tail
```
...
```bash
bedrock-langsmith-trace$ pip install -r tests/requirements.txt --user
# unit test
bedrock-langsmith-trace$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
bedrock-langsmith-trace$ AWS_SAM_STACK_NAME="bedrock-langsmith-trace" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "bedrock-langsmith-trace"
```
